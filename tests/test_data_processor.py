...
def test_columns_present(processed_df, project_config):
    assert all(col in processed_df.columns for col in project_config.num_features + [project_config.target, "id"]), "Not all required columns are present after preprocessing"


def test_id_is_string(processed_df, column_name="id"):
    assert all(isinstance(x, str) for x in processed_df[column_name]), f"Column '{column_name}' contains non-string values."
