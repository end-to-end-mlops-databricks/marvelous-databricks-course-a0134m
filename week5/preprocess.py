...
# Load source_data table - mimicking arrival of new data
""" Source data is synthetic data created using test and train set tables which are already processed so source data is also processed.
If source data was 'raw' data then we should run the data_preprocessor class to clean the raw data before appending to test train tables."""
...
if affected_rows_train > 0 or affected_rows_test > 0:
    spark.sql(f"""
        ...
        INSERT INTO {catalog_name}.{schema_name}.wine_features
        SELECT cast(id as string) as Id, volatile_acidity, alcohol, sulphates
        FROM {catalog_name}.{schema_name}.train_set
